  ...
  useEffect(() => {
    const fetchOrders = async () => {
      ...
      try {
        ...
        // Filter non-overdue orders (dueDate on or after current date or no dueDate)
        const currentDate = new Date();
        const nonOverdueOrders = ordersArray
          .filter(order => !order.dueDate || new Date(order.dueDate) >= currentDate)
          // Sort by dueDate (most recent first) and get the 5 most recent
          .sort((a, b) => {
          ...
          .slice(0, 5);
        setOrders(nonOverdueOrders);

        // Fetch customer names for each order
        const customerIds = [...new Set(nonOverdueOrders.map((order) => order.customerId))];
